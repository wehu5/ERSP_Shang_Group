...
def plotConfusionMatrix(data_dir, predictions, dataset_name, orig_class_names, threshold, mask, cluster_method, chosen_doc_indices):
    ...
    # Evaluate predictions only against ground truth of the chosen documents by getting list of only those documents' labels
    chosen_ground_truth = []
    for chosen in chosen_doc_indices:
        chosen_ground_truth.append(labels[chosen])

    #Calculate confusion matrix using ground truth vs cluster predictions
    conf_matrix = confusion_matrix(chosen_ground_truth, predictions, labels=list(range(len(orig_class_names))))
    ...
    # plt.show()
    # save_path = f"../tsne_output/Clustering experiments (malcolm)/hybrid clustering/doubleThresholds_maskingFixed_{dataset_name}_{threshold},{mask},{cluster_method}.png"
...
def importDataset(dataset_name,
    ...
    nytTopics_4_23_22 = os.path.join(data_dir, f"document_repr_lm-{lm_type}-{document_repr_type}.pk")
    with open(nytTopics_4_23_22, "rb") as f:
...
def partitionDataset(threshold, doc_reps, known_class_reps):
    ...
    for i,doc_rep in enumerate(doc_reps):
        ...
        if doc_max_similarity >= (1 - threshold):
            high_conf_docs.append(doc_tuple)
        elif doc_max_similarity <= threshold:
            low_conf_docs.append(doc_tuple)

    #Check partition sizes
    print(f"Confidence thresholds: High => {1-threshold}, Low => {threshold}")
    ...
    print(f"Percentage of total docs chosen = {(len(low_conf_docs) + len(high_conf_docs))/len(doc_reps)}%")
...
def evalCos(doc_reps, known_class_reps, dataset_name, orig_class_names):
    ...
    # hist_bins_labels = [str(x) for x in hist_bins]
    ...
    for index,class_name in enumerate(orig_class_names):
        ...
        # axes.flat[index].set_xticklabels(xticklabels) # , rotation=-45
        axes.flat[index].hist(class_values, bins=hist_bins)

        
        # class_values = class_dict[index] # List of max cosine similarities of docs in class_name
        # plt.hist(class_values, bins = hist_bins)
        # plt.title(f"Cosine similarities for '{class_name}'")
        # plt.xticks(hist_bins, rotation=-45)
        # plt.show()
    fig.suptitle("Cosine similarities for {raw/no-attention, NYT-Topics, 50% masking}")
    plt.show()
...
def main(dataset_name,
    ...
    # confidence_thresholds = [ 0.05, 0.15, 0.25, 0.35, 0.50, 0.65, 0.80 ]
    confidence_thresholds = [0.05, 0.10, 0.20, 0.30, 0.40]
    # confidence_thresholds = [0.05, 0.10, 0.20]
    for threshold in confidence_thresholds:
        '''
        Evaluation here!
        '''
        evalCos(doc_reps, known_class_reps, dataset_name, orig_class_names)
        '''
        Evaluation done.
        '''
        #Partition dataset
        ...
        low_conf_doc_predictions = kmeans.predict(low_conf_doc_reps) + len(known_class_names) # Indices accounting for known classes
        ...
        # Update document_class_assignment with low confidence kmeans predictions instead of cosine-to-known-classes predictions
        for j,doc_tuple in enumerate(low_conf_docs):
            prediction = low_conf_doc_predictions[j]
            doc_index = doc_tuple[1]
            document_class_assignment[doc_index] = prediction

        # Get list of only the chosen documents' indices
        chosen_doc_indices = []
        # HIGHCONF = 1
        # LOWCONF = 0
        for doc_tuple in low_conf_docs:
            doc_index = doc_tuple[1]
            chosen_doc_indices.append(doc_index)
        for doc_tuple in high_conf_docs:
            doc_index = doc_tuple[1]
            chosen_doc_indices.append(doc_index)
        chosen_doc_indices.sort()
        print(f"chosen_doc_indices[:30] = {chosen_doc_indices[:10]}")

        final_predictions = []
        for chosen in chosen_doc_indices:
            final_predictions.append(document_class_assignment[chosen])
            # for i,chosen_tuple in enumerate(chosen_doc_indices):
            #     if i<5:
            #         print(f"i = {i}, chosen_tuple = {chosen_tuple}")
            #     doc_index = chosen_tuple[0]
            #     conf = chosen_tuple[1]

            #     if conf == HIGHCONF:
            #         final_predictions.append(document_class_assignment[i])
            #     elif conf == LOWCONF:

            #     else:
            #         print("confidence type error")
            #         return
            
        # final_predictions = document_class_assignment
        ...
        #     final_predictions[doc_index] = kmeans_prediction 
        ...
        plotConfusionMatrix(data_dir, final_predictions, dataset_name,
                            orig_class_names, threshold, mask, cluster_method, chosen_doc_indices)
