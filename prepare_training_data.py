...
def write_to_dir(text, labels, dataset_name, suffix):
    assert len(text) == len(labels)
    # suffix = "pca64.clusgmm.bbu-12.mixture-100.42"
...
def main(dataset_name, cluster, mask_level, confidence_threshold, cos_threshold):
    ...
    suffix = f"pca64.{cluster}.bbu-12.mixture-100.42_{mask_level}_{cos_threshold}"
    ...
    ###
    write_to_dir(text, classes, dataset_name, f"{suffix}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cluster", default="clusgmm")
    parser.add_argument("--mask_level", type=str, default="50%")
    parser.add_argument("--cos_threshold", type=str, default="0.15")
    parser.add_argument("--confidence_threshold", default=0.5)
    args = parser.parse_args()
    print(vars(args))
    main(args.dataset_name, args.cluster, args.mask_level, args.confidence_threshold, args.cos_threshold)